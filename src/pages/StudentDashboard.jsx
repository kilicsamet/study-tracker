...
export default function StudentDashboard() {
  ...
  async function flushSync() {
    ...
    try {
      ...
      setDone((fresh.target ?? TARGETS.full.seconds) > 0 ? (fresh.totalSeconds || 0) >= (fresh.target ?? TARGETS.full.seconds) : false)
    } catch (err) {
  ...
  function beginLoops(data) {
    ...
    tickRef.current = setInterval(() => {
      ...
      if (prevHour.current !== null && hr > prevHour.current && !snoozed) {
  ...
  async function handleStart() {
    if (saving) return
    if (mode === ST.RUNNING) return
    if (targetType === 'holiday') return
                ...
                <div style={S.ringWrap}>
                  <svg viewBox="0 0 260 260" style={S.svg}>
                    ...
                    {[1, 2, 3, 4, 5].map((h) => {
                      ...
                      const x1 = 130 + (R - 8) * Math.cos(rad)
                      const y1 = 130 + (R - 8) * Math.sin(rad)
                      const x2 = 130 + (R + 4) * Math.cos(rad)
                      const y2 = 130 + (R + 4) * Math.sin(rad)
                    ...
                    {mode === ST.RUNNING &&
                      ...
                      (() => {
                        ...
                        return (
                          <circle cx={x} cy={y} r="7" fill="var(--rose)">

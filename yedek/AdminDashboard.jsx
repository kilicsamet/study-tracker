...
export default function AdminDashboard() {
  ...
  const grandTotal = days.reduce((s, d) => s + (d.totalSeconds || 0), 0)
  const done = days.filter((d) => d.done).length
  const avg = days.length ? grandTotal / days.length : 0
      ...
      <main style={S.main}>
        ...
        {!loading && tab === 'overview' && (
          <div className="anim-fadeup">
            <div style={S.kpiGrid}>
              {[
                { icon: '🕒', val: fmtHM(todayTotal), label: 'Bugün', c: 'var(--rose)' },
                { icon: '⏱', val: fmtHM(grandTotal), label: 'Genel toplam', c: 'var(--lavender)' },
                { icon: '✅', val: `${done} gün`, label: 'Tamamlanan gün', c: 'var(--ok)' },
                { icon: '📈', val: fmtHM(avg), label: 'Günlük ortalama', c: 'var(--gold)' },
                { icon: '📅', val: fmtHM(l7total), label: 'Son 7 gün', c: 'var(--mint)' },
              ].map((k, i) => (
